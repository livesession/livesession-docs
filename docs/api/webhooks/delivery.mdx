...
## Overview
When an event ready for [webhook](https://livesession.io) transmission occurs within LiveSession, an [event payload](#) is crafted.
...
Once an event is dispatched, a [payload signature](https://livesession.io) is computed and included in the request.
...
LiveSession [limits](https://livesession.io) the number of events sent for a given webhook to a maximum of *1000 per minute*.

If you've configured an endpoint for a high-volume event flow, like a frequently triggered custom event during [user sessions](https://livesession.io),
expect possible webhook delays.
When the number of events for an endpoint exceeds *1000 per minute*, the extra events will be [queued](https://livesession.io).
