...
This verification can be accomplished by calculating a digital [signature](https://livesession.io).

Every webhook request incorporates a [base64-encoded HMAC-SHA256](https://livesession.io) signature included in `LiveSession-Signature` header.
This header is created using the [webhook's secret](https://livesession.io) in conjunction with the data transmitted in the request.
